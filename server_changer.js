(function () {
    ...
    function ServerSwitcher() {
        ...
        this.init = function () {
            // 1. Додаємо в налаштування (Інше) - Виправлено дублювання
        ...
        this.addStyles = function() {
            var css = `
                ...
                /* Поточний сервер - Жовтий */
                .server-item.active-server { border-color: #ffc107; background: rgba(255, 193, 7, 0.1); pointer-events: none; }
                .server-item.active-server .server-name { color: #ffc107; }
                
                /* Кольори назв залежно від статусу */
                .server-item.is-online .server-name { color: #00ff44; }
                .server-item.is-offline .server-name { color: #ff0044; }
                
                /* Крапки статусу */
                .server-status { width: 12px; height: 12px; border-radius: 50%; background: #555; }
                .server-status.online { background: #00ff44; box-shadow: 0 0 10px #00ff44; width: 12px !important; height: 12px !important; }
        ...
        this.checkAvailability = function(url, callback) {
            ...
            var id = setTimeout(() => controller.abort(), 3000);
            fetch(url + '?t=' + Date.now(), { mode: 'no-cors', signal: controller.signal })
        ...
        this.open = function () {
            ...
            // 1. Поточний сервер окремо згори
            var current_server_obj = servers.find(s => s.url.indexOf(current_host) > -1);
            if(current_server_obj) {
                list.append(`
                    <div class="server-item active-server">
                        <div class="server-info">
                            <span class="current-label">Поточний</span>
                            <div class="server-name">${current_server_obj.name}</div>
                        </div>
            ...
            // 2. Список інших серверів
            servers.forEach(function (server) {
                if (current_server_obj && server.url === current_server_obj.url) return;
                ...
                _this.checkAvailability(server.url, function(is_online){
                    ...
                    if(is_online) {
                        dot.addClass('online');
                        item.addClass('is-online');
                    } else {
                        dot.addClass('offline-dot');
                        item.addClass('is-offline');
                    }
                });

                item.on('hover:enter', function () {
                    if($(this).hasClass('is-offline')) return Lampa.Noty.show('Сервер недоступний');
                    list.find('.server-item').css('border-color', 'transparent');
            ...
            btn.on('hover:enter', function () {
                ...
                // Android Fix: запис в Storage перед рестартом
